...
- **2026-05 — Router LLM extracted plausible-but-wrong field names (Build 21).** `_registry_summary()` sent only name/tier/desc — no Input schema. LLM guessed `prospect_name`, `industry` instead of `business_name`, `business_type`. Lesson: every router-style LLM call producing structured output must be schema-grounded — via tool use or schema-in-prompt — never free-form. Fix: include `cls.Input.model_json_schema()` fields in registry summary.
- **2026-05 — Tests passed but missed schema grounding (Build 21).** Build 17/18/20 tests used hardcoded field names so they exercised wiring but not field-name correctness. Lesson: integration tests for router output must exercise multiple natural-language inputs and assert schema conformance, not just that inputs were passed.
- **2026-05 — result.reason captured internally but surfaced nowhere (Build 21).** Failure reason was stored in MeeseeksResult but not emitted to foreman user output, debug channel, or journalctl. Lesson: state worth capturing in Result fields must be surfaced in EVERY observability layer — user output, debug channel, logs. Otherwise it's invisible state.
- **2026-05 — Tool functions existing ≠ tools wired to LLM (Build 23).** `toolkits=["research"]` declared intent; `firecrawl_scrape` and `http_fetch` were implemented as Python functions; but `subprocess_runner.py` never passed them to the API. The LLM was told to call tools it had no mechanism to call. Lesson: every meeseeks toolkit declaration needs a runtime assertion (or integration test) proving it translates to actual LLM tool calls. Declaration without verification is invisible non-wiring.
